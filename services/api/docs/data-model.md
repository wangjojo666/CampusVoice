...
| `user_settings`                              | User-editable profile and preferences: major, grade, course/teacher JSON, default reminder and timezone. The legacy ASR JSON column is not user-writable. | One row per user; reminder is non-negative. ASR provider/model/device are read-only server runtime status. |
