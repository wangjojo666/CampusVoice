...
test("wx.login code exchange stores only the bounded app session", async () => {
  requestHandler = (options) => {
    ...
    options.success({
      statusCode: 200,
      data: {
        session_token: "session-token-value-with-more-than-32-characters",
  ...
  assert.equal(
    ...
    "session-token-value-with-more-than-32-characters",
  );
...
test("authenticated API requests attach bearer token and reject unsafe paths", async () => {
  storage.set("campusvoice.wechatSession", {
    token: "existing-session-token-value-with-32-characters",
  ...
  requestHandler = (options) => {
    ...
    assert.equal(
      ...
      "Bearer existing-session-token-value-with-32-characters",
    );
...
test("an uncertain write retry reuses its key and clears it only after success", async () => {
  storage.set("campusvoice.wechatSession", {
    token: "retry-session-token-value-with-32-characters",
    expiresAt: new Date(Date.now() + 3_600_000).toISOString(),
    displayName: "微信用户",
    apiBase: "https://api.example.edu",
  });
  ...
  assert.deepEqual(Object.keys(pending[0]).sort(), ["apiBase", "createdAt", "fingerprint", "key"]);
...
test("corrupt pending-write storage fails closed before any network request", async () => {
  storage.set("campusvoice.wechatSession", {
    token: "corrupt-session-token-value-with-32-characters",
    expiresAt: new Date(Date.now() + 3_600_000).toISOString(),
    displayName: "微信用户",
    apiBase: "https://api.example.edu",
  });
...
test("pending-write storage read failures fail closed before any network request", async () => {
  storage.set("campusvoice.wechatSession", {
    token: "read-error-session-token-value-with-32-characters",
    expiresAt: new Date(Date.now() + 3_600_000).toISOString(),
    displayName: "微信用户",
    apiBase: "https://api.example.edu",
  });
...
test("a definite write rejection clears the pending idempotency key", async () => {
  storage.set("campusvoice.wechatSession", {
    token: "definite-session-token-value-with-32-characters",
    expiresAt: new Date(Date.now() + 3_600_000).toISOString(),
    displayName: "微信用户",
    apiBase: "https://api.example.edu",
  });
...
test("an aged outcome-uncertain write freezes without deleting its evidence", async () => {
  storage.set("campusvoice.wechatSession", {
    token: "aged-session-token-value-with-32-characters",
    expiresAt: new Date(Date.now() + 3_600_000).toISOString(),
    displayName: "微信用户",
    apiBase: "https://api.example.edu",
  });
...
test("a persisted outcome-uncertain write is never replayed after runtime restart", async () => {
  storage.set("campusvoice.wechatSession", {
    token: "restart-session-token-value-with-32-characters",
    expiresAt: new Date(Date.now() + 3_600_000).toISOString(),
    displayName: "微信用户",
    apiBase: "https://api.example.edu",
  });
...
test("a stale success callback cannot clear a newer same-write idempotency key", async () => {
  storage.set("campusvoice.wechatSession", {
    token: "concurrent-session-token-value-with-32-characters",
    expiresAt: new Date(Date.now() + 3_600_000).toISOString(),
    displayName: "微信用户",
    apiBase: "https://api.example.edu",
  });
...
test("pending write intents are scoped to one API base and never clear another base", async () => {
  ...
  const writeAttempts = [];
  storage.set("campusvoice.wechatSession", {
    token: "api-a-session-token-value-with-32-characters",
    expiresAt: new Date(Date.now() + 3_600_000).toISOString(),
    displayName: "A 用户",
    apiBase: "https://api.example.edu",
  });
  ...
  storage.set("campusvoice.wechatSession", {
    token: "api-b-session-token-value-with-32-characters",
    expiresAt: new Date(Date.now() + 3_600_000).toISOString(),
    displayName: "B 用户",
...
test("a delayed 401 cannot clear a replacement session and anonymous 401 clears none", async () => {
  storage.set("campusvoice.wechatSession", {
    token: "old-request-session-token-value-with-32-characters",
    expiresAt: new Date(Date.now() + 3_600_000).toISOString(),
    displayName: "旧用户",
    apiBase: "https://api.example.edu",
  });
  ...
  const replacement = {
    token: "replacement-session-token-value-with-32-characters",
    expiresAt: new Date(Date.now() + 3_600_000).toISOString(),
    displayName: "新用户",
    apiBase: "https://api.example.edu",
  };
...
test("a delayed logout callback cannot clear a newly established session", async () => {
  storage.set("campusvoice.wechatSession", {
    token: "logout-old-session-token-value-with-32-characters",
    expiresAt: new Date(Date.now() + 3_600_000).toISOString(),
    displayName: "旧用户",
    apiBase: "https://api.example.edu",
  });
  ...
  assert.equal(storage.has("campusvoice.wechatSession"), false);
  const replacement = {
    token: "logout-new-session-token-value-with-32-characters",
    expiresAt: new Date(Date.now() + 3_600_000).toISOString(),
    displayName: "新用户",
    apiBase: "https://api.example.edu",
  };
...
test("a stale 401 cannot cancel a same-base login already in flight", async () => {
  storage.set("campusvoice.wechatSession", {
    token: "stale-session-token-value-with-32-characters",
    expiresAt: new Date(Date.now() + 3_600_000).toISOString(),
    displayName: "旧用户",
    apiBase: "https://api.example.edu",
  });
  ...
  loginExchange.success({
    statusCode: 200,
    data: {
      session_token: "fresh-session-token-value-with-32-characters",
      expires_at: new Date(Date.now() + 3_600_000).toISOString(),
      display_name: "新用户",
    },
  });
  ...
  assert.equal(fresh.token, "fresh-session-token-value-with-32-characters");
...
test("a session token is never sent to a different configured API base", async () => {
  storage.set("campusvoice.wechatSession", {
    token: "old-origin-session-token-value-with-32-characters",
    expiresAt: new Date(Date.now() + 3_600_000).toISOString(),
    displayName: "旧服务用户",
  ...
  requestHandler = (options) => {
    if (options.url.endsWith("/api/auth/wechat/login")) {
      ...
      options.success({
        statusCode: 200,
        data: {
          session_token: "new-origin-session-token-value-with-32-characters",
          expires_at: new Date(Date.now() + 3_600_000).toISOString(),
          display_name: "新服务用户",
        },
      });
    ...
    assert.equal(
      options.header.Authorization,
      "Bearer new-origin-session-token-value-with-32-characters",
    );
...
test("a pending login cannot commit after the configured API base changes", async () => {
  ...
  deferredExchange.success({
    statusCode: 200,
    data: {
      session_token: "stale-login-session-token-value-with-32-characters",
      expires_at: new Date(Date.now() + 3_600_000).toISOString(),
      display_name: "旧服务用户",
    },
  });
