...
test("01 今天面板展示真实任务与日程，并渐进披露已验证操作", async ({ page }) => {
  ...
  const weekly = page.getByRole("region", { name: "一周节奏" });
  await expect(today).toBeVisible();
  await expect(weekly).toBeVisible();
  await expect(today.getByText("复习机器学习", { exact: true }).first()).toBeVisible();
  await expect(today.getByText("机器学习研讨课", { exact: true })).toBeVisible();
...
test("03 新建待办经过 UI 核对并携带服务端写挑战", async ({ page }) => {
  ...
  await expect(page.locator(".verified-finish")).toHaveCount(0);
  await dialog.getByRole("button", { name: "确认并保存" }).click();
  await expect(page.getByRole("status")).toContainText("待办已保存并验证");
  const verifiedFinish = page.locator(".verified-finish");
  await expect(verifiedFinish).toContainText("这一步稳稳落地了");
  await expect(verifiedFinish).toHaveCSS("animation-name", "none");
  await expect(verifiedFinish).toHaveCSS("transform", "none");
